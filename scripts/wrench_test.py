...
class WrenchTest():
    def __init__(self) -> None:
        rospy.init_node('wrench_test',anonymous=True)
        ...
        # self.tau_ext_sub = rospy.Subscriber("/tau_ext", Float64MultiArray, self.tau_ext_callback, queue_size=10)
        self.tau_commanded_sub = message_filters.Subscriber('/iiwa/CartesianImpedance_trajectory_controller/commanded_torques', Float64MultiArray)
        self.tau_ext_sub = message_filters.Subscriber('/tau_ext', Float64MultiArray)
        self.ts = message_filters.ApproximateTimeSynchronizer([self.tau_commanded_sub,self.tau_ext_sub],queue_size=10,slop=0.1,allow_headerless=True)
        self.ts.registerCallback(self.tau_diff_callback)
    ...
    def tau_diff_callback(self,tau_cmd_msg, tau_ext_msg):
        tau_diff = tau_cmd_msg.data-tau_ext_msg.data
        print(tau_diff)
    ...
    # def tau_ext_callback(self,tau_ext_msg):
    #     print("Im in")
    #     tau_diff = tau_ext_msg.data
    #     print(tau_diff)
